...
class BicubicDownDataset(data.Dataset):
    def initialize(self, dataset_opt):
        ...
        # If subset is used, it is assumed that there is a txt file named with 'subset.txt' in dataroot. Only for train phase
        if dataset_opt.use_subset: # Don't use by default
    ...
    def __getitem__(self, index):
        ...
        H_tensor = self.to_tensor(H_img) # HR image is in range [0,1]
        L_tensor = self.to_tensor(L_img) # LR image is in range [0,1]
