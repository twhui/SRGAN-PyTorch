...
def act(act_type, **kwargs):
    ...
    if act_type == 'relu':
        ...
        layer = nn.Tanh(**kwargs)
    elif act_type == 'sigmoid':
        layer = nn.Sigmoid(**kwargs)
    elif act_type == 'softmax':
        layer = nn.Softmax(dim=1, **kwargs)
    else:
...
"""
Tranposed Conv2d Layer with normalization, activation, dropout
"""
class TConv2dBlock(nn.Module):
    def __init__(self, input_nc, output_nc, kernel_size, stride=1, crop_pad_size=0, output_pad_size=0, dilation=1, groups=1, bias=True,
                 norm_type=None, act_type='relu', use_dropout=False):
        super(TConv2dBlock, self).__init__()
        self.C = nn.ConvTranspose2d(input_nc, output_nc, kernel_size, stride, crop_pad_size, output_pad_size, groups, bias, dilation)
        self.N = norm(norm_type, output_nc) if norm_type else identity
        self.A = act(act_type) if act_type else identity
        self.D = dropout() if use_dropout else identity
        self.weight_init()
...
def ResNetBlock(input_nc, mid_nc, output_nc, kernel_size=3, stride=1, bias=True,
    ...
    conv2 = Conv2dBlock(mid_nc,   output_nc, kernel_size, stride, bias=bias, pad_type=pad_type, norm_type=norm_type, act_type=None,     use_dropout=use_dropout)
